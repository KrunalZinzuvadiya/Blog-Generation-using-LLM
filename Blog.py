...
st.title("Blog Generation using LLM...🤖")
st.header("Create a Blog as per your size and keywords at free...")
...
with title_expander: # Create a content block within the title expander
    ...
    num_of_words = st.slider('Number of Words', min_value=200, max_value=5000, step=50) # Slider for selecting the desired number of words
    ...
    if keyword_button: # Handle button click for adding keyword
        ...
        for keyword in st.session_state['keywords']:  # Display the current list of keywords
            ...
            st.write(f"<div style='display: inline-block; background-color: orange; padding: 5px; margin: 5px;'>{keyword}</div>", unsafe_allow_html=True)
